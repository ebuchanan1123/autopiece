...
import { createListing } from "@/src/features/listings/listings.api";
...
export default function SellerListingNewScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setForm((current) => ({
          ...current,
          address: seller.address || current.address,
          city: seller.city || current.city,
          wilaya: seller.wilaya || current.wilaya,
          lat: seller.lat ?? current.lat,
          lng: seller.lng ?? current.lng,
        }));
      } catch {}
    })();
  }, []);
  ...
  async function save() {
    if (!canSubmit) {
      Alert.alert("Missing details", "Please fill in the listing basics before publishing.");
      return;
    }
    ...
    try {
      ...
      Alert.alert("Listing created", "Your new bag is now live.");
      router.replace("/(app)/seller-dashboard");
    } catch (e: any) {
      Alert.alert("Could not create listing", e?.message ?? "Please try again.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <>
      <ScreenHeader title="Create a listing" showBack />
      <ScreenBody>
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          <View style={styles.heroCard}>
            ...
            <Pressable style={styles.heroButton} onPress={uploadBanner} disabled={uploading}>
              <Text style={styles.heroButtonText}>{uploading ? "Uploading..." : "Add banner image"}</Text>
            </Pressable>
          </View>
          ...
          <Pressable style={[styles.saveBtn, !canSubmit ? styles.saveBtnDisabled : null]} onPress={save} disabled={saving || !canSubmit}>
            <Text style={styles.saveBtnText}>{saving ? "Publishing..." : "Publish listing"}</Text>
          </Pressable>
        </ScrollView>
      </ScreenBody>
    </>
